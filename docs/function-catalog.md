...
| `EuropeanCallPricer` | `crates/mc-core/src/runtime/cpu.rs` | Expressive builder-style API for configuring and executing the current CPU European-call runtime. | Inputs: chained builder methods. Output: `EuropeanCallResult` through `price()`. | Deterministic for identical configuration and method. | Focused on one workload family for now. | Strong ergonomic candidate for future Python and agent wrappers. |
