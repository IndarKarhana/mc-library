...
| `NativeArtifactMetadata` | `crates/mc-core/src/backend/mod.rs` | Describe staged native kernel artifacts for GPU backends. | Output fields include kernel family, entry point, source module, compile request status, compile success status, and optional compiled module path. | Deterministic for same build and environment. | Compile success still depends on toolchain availability; native execution is not enabled yet. | Strong future agent/tool diagnostic surface. |
...
| `AppleMetalBackend::execute` | `crates/mc-core/src/backend/metal.rs` | Execute the staged v1 Metal kernel path on macOS when `metal-native` is enabled and runtime compilation succeeds, otherwise fall back to CPU-backed backend execution. | Inputs: `CompiledArtifact`, `BackendExecutionInput::EuropeanCall`. Output: `RunOutput`. | Deterministic for identical config and seed. | Native path is currently narrow, uses CPU-generated normals, and falls back when runtime support is unavailable. | Strong future tool/runtime surface, but current scope is intentionally narrow. |
