...
| `MonteCarloTechnique` | `crates/mc-core/src/runtime/cpu.rs` | Select variance-reduction technique for the current CPU European-call runtime. | Input enum. Output: controls runtime behavior. | Deterministic. | Current support is `standard` and `antithetic` only. | Good future tool parameter surface. |
